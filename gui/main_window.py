...
class RedHawkGUI:
    ...
    def export_report(self):
        ...
        output_path = self.current_results.get('output_path')
        ...
        if not output_path:
            ...
            if reports_dir.exists():
                ...
                if json_files:
                    ...
                    self.log(f"[*] Using latest report: {latest.name}")
                    
                    # Also update current_results with this path
                    self.current_results['output_path'] = output_path
        
        if not output_path:
            # No output_path and no report files found
            ...
            if discovery_files:
                ...
                messagebox.showinfo(
                    ...
                    f"The scan completed but report file wasn't found in expected location.\n\n"
                    f"To get a report:\n"
        ...
        if filepath:
            try:
                self.engine.generate_report(output_path, 'html')
                # Copy the generated HTML to the chosen location
                html_file = output_path.replace('.json', '.html')
                if Path(html_file).exists():
                    shutil.copy(html_file, filepath)
                    messagebox.showinfo("Success", f"Report exported to:\n{filepath}")
                else:
                    messagebox.showerror("Error", "HTML report was not generated properly")
            except Exception as e:
