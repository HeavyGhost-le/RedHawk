"""
RedHawk Advanced GUI - Enhanced Version with Phase 1 Features
Features:
...
class RedHawkGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("RedHawk Advanced - Offensive Security Framework")
        ...
        self.target_history = self.load_target_history()
    ...
    def save_scan_history(self, scan_info):
        """Save scan to history"""
        try:
            self.scan_history.insert(0, scan_info)
            self.scan_history = self.scan_history[:100]  # Keep last 100
            with open('scan_history.json', 'w') as f:
                json.dump(self.scan_history, f, indent=2)
        except Exception as e:
            print(f"Failed to save history: {e}")
    
    def load_target_history(self):
        """Load target history"""
        try:
            history_file = Path('target_history.json')
            if history_file.exists():
                with open(history_file) as f:
                    return json.load(f)
        except:
    ...
    def load_filter_presets(self):
        ...
        try:
            presets_file = Path('filter_presets.json')
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(title_frame, text="🦅 RedHawk Advanced Security Framework", 
                               style='Title.TLabel')
        ...
        target_container = ttk.Frame(input_frame)
        target_container.pack(side=tk.LEFT, padx=5)
        
        self.target_var = tk.StringVar(value="example.com")
        self.target_entry = ttk.Combobox(target_container, textvariable=self.target_var,
                                         width=38, font=('Courier', 10))
        self.target_entry.pack(side=tk.LEFT)
        self.target_entry['values'] = self.target_history if self.target_history else ['example.com']
        self.target_entry.set("example.com")
